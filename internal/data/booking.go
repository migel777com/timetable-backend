...
func (m *BookingModel) GetAllByRoomDate(room int64, date time.Time) ([]*Booking, error){
	stmt := `SELECT id, room, room_id, reserver, reserver_id, reserver_info, reserver_email, day, date, start_time, end_time, reason, confirmed, created_time FROM booking WHERE room_id = ? AND DATE(date) = DATE(?) ORDER BY date;`
	rows, err := m.DB.Query(stmt, room, date)
	if err!=nil{
		return nil, err
	}
	defer rows.Close()
